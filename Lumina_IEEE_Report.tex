...
environments. GPT-2~\cite{radford2019gpt2}, the model used in this work,
requires approximately 500\,MB of memory for its 117 million
parameters---modest by modern standards, yet representative of the
architectural challenge: as models scale to billions of parameters, a single
consumer GPU is insufficient to host the entire model graph.
...
Lumina implements a two-node split inference pipeline for the GPT-2
transformer architecture. The system is designed around enterprise distributed
systems principles: services are independently deployable, communicate via
versioned REST APIs, self-register with a coordination service, and expose
health and observability endpoints. The project serves as a proof-of-concept
for distributed LLM inference deployable on AWS EC2 instances without
specialized networking or GPU hardware.
...
\begin{itemize}
  ...
  \item A memory-aware model loading strategy using per-layer \texttt{device\_map}
        assignments that maps unused transformer layers to PyTorch's \texttt{meta}
        device, eliminating peak RAM from non-executing weights.
...
Lumina comprises four independently deployed components: Node~A (head),
Node~B (tail), the Tracker coordination service, and a React frontend
dashboard. Figure~\ref{fig:arch} illustrates the high-level system
architecture.
...
\begin{tikzpicture}[
  node distance=0.5cm,
  box/.style={rectangle, rounded corners=3pt, draw, text width=3.6cm,
...
% Vertical stack: Frontend -> Client -> Node A -> Node B
%                                              -> Tracker (right of Node B)

\node[box, fill=purple!12]  (fe)     {React Frontend\\{\footnotesize port 80}};
\node[box, fill=gray!15,    below=of fe]     (client) {Client / Browser};
\node[box, fill=blue!15,    below=of client] (nodea)
  {\textbf{Node A} (Head)\\{\footnotesize port 8001}\\
   {\tiny layers 0$\to$split, tokenize}};
\node[box, fill=green!15,   below=of nodea]  (nodeb)
  {\textbf{Node B} (Tail)\\{\footnotesize port 8002}\\
   {\tiny layers split$\to$12, return token}};
\node[box, fill=orange!20,  right=1.2cm of nodea] (tracker)
  {\textbf{Tracker}\\{\footnotesize port 8003}\\
   {\tiny VRAM split, heartbeat}};

% Arrows
\draw[arrow]  (fe)     -- node[lbl, right]{/generate, /nodes} (client);
\draw[arrow]  (client) -- node[lbl, right]{POST /generate}    (nodea);
\draw[arrow]  (nodea)  -- node[lbl, right]{hidden states (b64)} (nodeb);
\draw[darrow] (nodeb)  -- node[lbl, left] {next token}
              ([xshift=-6pt]nodeb.north) -- ([xshift=-6pt]nodea.south);
\draw[darrow] (nodea.east) -- node[lbl, above]{GET /assignment}
              node[lbl, below]{POST /heartbeat} (tracker.west);
\draw[darrow] (nodeb.east) -| node[lbl, right, near end]{POST /register}
              (tracker.south);

\end{tikzpicture}
\caption{Lumina system architecture. Solid arrows: data flow. Dashed arrows:
...
an autoregressive generation loop. For each token position, it executes the
head portion of the GPT-2 transformer: the token embedding layer, positional
embedding, dropout, and the first \texttt{split\_layer} transformer blocks.
The resulting hidden-state tensor is serialized and forwarded to Node~B.

Before each token step, Node~A queries the Tracker's \texttt{/assignment}
endpoint to obtain the current \texttt{split\_layer} value. This per-token
querying ensures the system responds immediately to any rebalancing event
triggered by a change in node composition.

\subsection{Node B --- Tail Service}

Node~B exposes a single inference endpoint: \texttt{POST /forward\_tail}. It
receives the serialized hidden states along with the current
\texttt{split\_layer} index, executes the remaining transformer blocks
(\texttt{split\_layer} through layer~11), applies the final layer normalization
and language model head, and returns the argmax next token as a base64-encoded
tensor. Node~B maintains no generation state---all context tracking is
handled by Node~A.
...
\texttt{heartbeat\_timeout\_sec} (default: 30~s). When both active nodes are
present, the split layer is computed proportionally to their VRAM. A version
counter increments on every split change, allowing clients to detect assignment
drift without maintaining local state.
...
for _ in range(max_new_tokens):
    split = resolve_split_layer()
    hidden = run_head(ids, mask, split)
    payload = TailForwardRequest(
        hidden_states_b64=encode(hidden),
        split_layer=split, ...)
    resp = POST(node_b_url, payload)
    next_tok = decode(resp.token_ids_b64)
    ids = cat([ids, next_tok], dim=1)
    if next_tok == eos_token:
        break
\end{lstlisting}

This design means generation latency scales linearly with token count, with
each token incurring the full serialization and network transfer overhead. For
a 20-token generation, the system performs 20 HTTP round trips between
nodes---a deliberate tradeoff that keeps the implementation simple while
validating the core tensor transport contract.

\subsection{Service Self-Registration}

Both Node~A and Node~B register themselves with the Tracker on FastAPI startup
via the \texttt{on\_event('startup')} hook. The registration payload includes
the node's unique identifier, role designation, estimated VRAM, maximum layer
capacity, and total model layer count. This self-registration pattern eliminates
the need for static configuration files and allows new nodes to join the
cluster without restarting existing services.

\subsection{Memory-Aware Model Loading}

A key challenge in split inference is that loading the full model on each node
wastes the RAM of layers that will never execute. The
\texttt{model\_adapter} module addresses this with a \texttt{make\_device\_map}
function that constructs a per-layer device map before model loading. Layers
outside the node's assigned range \texttt{[keep\_start, keep\_end)} are mapped
to the \texttt{meta} device---a PyTorch virtual device that holds parameter
shapes but allocates no memory---while active layers are mapped to the real
compute device (CUDA or CPU).

\begin{lstlisting}[caption={Selective layer device mapping in \texttt{model\_adapter.py}.}, label={lst:devmap}]
def make_device_map(model_name, keep_start,
                    keep_end, target_device):
    config = AutoConfig.from_pretrained(model_name)
    device_map = {"": target_device}
    prefix = "transformer.h"   # GPT-2 layer prefix
    for i in range(config.n_layer):
        if not (keep_start <= i < keep_end):
            device_map[f"{prefix}.{i}"] = "meta"
    # Drop embeddings on tail nodes, lm_head on head
    if keep_start > 0:
        device_map["transformer.wte"] = "meta"
        device_map["transformer.wpe"] = "meta"
    if keep_end < config.n_layer:
        device_map["lm_head"] = "meta"
        device_map["transformer.ln_f"] = "meta"
    return device_map
\end{lstlisting}

This approach replaces the previous \texttt{prune\_to\_range} post-load pruning
strategy, which required loading the full model before discarding unused layers.
By passing the device map directly to \texttt{AutoModelForCausalLM.from\_pretrained},
unused weights are never materialized, reducing peak RAM consumption
proportionally to the fraction of layers assigned to each node. For a 12-layer
GPT-2 split 6/6, each node loads only half the transformer blocks plus its
required embeddings or head weights.
...
All three backend services share a single Docker image built from a
\texttt{python:3.11-slim} base. The \texttt{command:} directive in each
docker-compose service definition selects which uvicorn application to launch.
Docker Compose health checks enforce a strict startup order: the Tracker must
be healthy before Node~B starts, and both must be healthy before Node~A starts.
...
The system was validated end-to-end on AWS EC2 (\texttt{t3.small}, CPU-only)
using the GPT-2 model (12 transformer layers, 117M parameters). Functional
correctness was confirmed by verifying that text generated via the split
pipeline is identical to single-node generation with the same prompt and random
seed. The autoregressive loop correctly terminates on EOS token detection.

\subsection{Latency Profile}

On CPU-only \texttt{t3.small} instances, end-to-end generation latency for a
20-token prompt is dominated by transformer forward passes rather than
serialization overhead. The tensor transport pipeline
(\texttt{torch.save} $\to$ base64 encode $\to$ HTTP POST $\to$ base64 decode
$\to$ \texttt{torch.load}) adds approximately 15--40\,ms per token for GPT-2
hidden states (shape: $[1, L, 768]$ where $L$ is sequence length). Per-token
inference time on CPU is approximately 200--400\,ms, making serialization
overhead a secondary concern in this hardware configuration.

The primary performance observation is that autoregressive generation requires
$\mathcal{O}(N)$ network round trips for $N$ tokens. Let $T_{\text{head}}$,
$T_{\text{serial}}$, $T_{\text{net}}$, and $T_{\text{tail}}$ denote the time
for head inference, serialization, network transfer, and tail inference
respectively. The total generation latency for $N$ tokens is:

\begin{equation}
  T_{\text{total}} = N \cdot \left(T_{\text{head}} + T_{\text{serial}}
                     + T_{\text{net}} + T_{\text{tail}}\right)
  \label{eq:latency}
\end{equation}

\subsection{Dynamic Split Behavior}

With two \texttt{t3.small} instances reporting equal VRAM (4.0\,GB each, the
CPU fallback value), the Tracker assigns \texttt{split\_layer}~$= 6$, exactly
halving the 12 GPT-2 layers. When a node registers with a larger VRAM value
(simulated in unit tests), the split shifts proportionally. The version counter
correctly detects changes, allowing Node~A to detect stale assignments without
...
Lumina's three backend services map cleanly onto the microservices decomposition
pattern~\cite{newman2021microservices}. Each service owns a distinct
responsibility: Node~A orchestrates generation, Node~B performs computation,
and the Tracker manages coordination. Services communicate exclusively via
well-defined REST APIs with Pydantic-validated request/response schemas,
ensuring contract stability as individual services evolve independently.
...
(30~s) and removed from the active assignment. Second, the fallback split
layer: when no active tail node is registered, the Tracker returns the static
